...
Every main view shows a global `Papercuts N` header indicator for the open count. A zero count stays muted. Press `i`, or click the indicator, to open the temporary Papercuts utility panel. Press `i` or `Esc` to close it and restore the current main view with its selection, focus, filters, arrangement, and scroll unchanged. Use `j`/`k`, arrows, `g`/`G`, and page keys for local navigation. `Enter` opens the selected Papercut detail. Tab/Shift-Tab or `h`/`l` changes list/detail focus; Enter never returns to the list. Mouse row selection, pane focus, and wheel scrolling mirror those controls.
