...
| --- | --- |
| **Board / State Board**<br>*[render or screenshot placeholder]* | Active tasks grouped by `todo`, `in-progress`, and `validation`, plus a flat overlapping Papercuts tag lens. Use it to select tasks, inspect hierarchy, filter work, and move through the workflow. |
...
| **Papercuts utility panel**<br>*[render or screenshot placeholder]* | A temporary active-Papercut list and detail pane over the current main view. It is a read-only global inbox, not a fifth top-level view. The Board also has its own filtered Papercuts subview. |
...
Every main view shows a global `Papercuts N` header indicator for the active count. The indicator is always the unfiltered total, even while Board filters are active. A zero count stays muted. Press `i`, or click the indicator, to open the temporary Papercuts utility panel. Press `i` or `Esc` to close it and restore the current main view with its selection, focus, filters, arrangement, and scroll unchanged. Use `j`/`k`, arrows, `g`/`G`, and page keys for local navigation. `Enter` opens the selected Papercut detail. Tab/Shift-Tab or `h`/`l` changes list/detail focus; Enter never returns to the list. Mouse row selection, pane focus, and wheel scrolling mirror those controls.

The Board's Papercuts subview is a separate, filtered surface: a flat overlapping tag lens over the active Board tasks tagged `papercut`. Both surfaces derive from the same active Papercut-tagged task documents; the header indicator and `i` panel remain the unfiltered global view, while the Board subview follows the active Board filters.

The panel lists active Papercut-tagged tasks and renders ID, title, state, Accord status, optional tags and references, created/updated timestamps, path, and styled Markdown body. It is read-only. Add, edit, resolve, reopen, delete, and task-promotion actions stay in the CLI or integration tools. A workspace with no Papercut-tagged tasks is an empty inbox. A malformed record appears as a panel warning without blocking Board, Logs, Rules, or Decisions. Normal manual and watched reloads include Papercut task files.

Board supports explicit filter (`f`), the `a` Actions picker for native lifecycle transitions, and context-aware Validation (`v`) pickers, reloads, mouse selection and scrolling, inline hierarchy, row previews, and task editing. Press `b` to switch between the State Board and Epic Board. `Enter` is the one row activation key: it expands or collapses hierarchy parents and opens or closes the inline preview for leaf rows and Epic Board rows. Clicking an already-selected row uses the same activation path. Space has no Board action.
...
Mouse hit regions are bounded and mirror safe keyboard actions. Click top tabs to switch views, the global Papercuts count to open or close its panel, Board state tabs to switch states, and Papercut/Board/Logs/Decisions rows to select items. Click an already-selected Board row to activate it with the same semantics as Enter. The Board footer stays a concise local command strip: Add, Edit, Filter, Move, Validate, arrangement toggle, and Help. State/count context stays in the state tabs instead of being repeated in the list border or footer. Temporary status messages expire; the persistent Papercuts count remains in the upper utility/status gutter. Footer actions use the matching keyboard behavior where safe. The wheel scrolls the pane under the pointer; other regions are no-ops. Drag and drop is not supported in v0.

## Board, hierarchy, and filtering

The State Board's final subview is **Papercuts**, a flat overlapping tag lens. It lists every active Board task tagged `papercut` once in document order, including matches nested beneath a non-Papercut parent and matches whose parent is in another workflow state, so no counted match requires ancestor expansion to be seen. Each row shows its hierarchy parent (`parentId`) as muted `↳` context with the resolved parent role and title where space permits. `references` are loose links: they never create hierarchy, membership, or nesting, so a referenced-only task still appears as its own flat row. Selecting a flat row opens that exact record's inline preview or detail pane, which keeps `Parent`/`Subtask of` and `References` as distinct fields. Papercut-tagged tasks also remain in their own workflow-state tabs and in the Epic Board, so Papercuts is an overlapping view rather than an exclusive partition.

State and Papercuts tab labels show the count matching the active Board filters. When filters reduce a view, the label also shows the unfiltered total for that view, such as `PAPERCUTS 2/5`, so filtered and global totals stay distinguishable. Clearing filters restores the single total.
...
The Board presents the active `todo`, `in-progress`, and `validation` states. `validation` is the review queue; it is not a permanent completion state. Papercut-tagged tasks keep their own workflow state and appear both in that state's tab and in the flat overlapping Papercuts lens; the lens is a tag view, not a workflow state or hierarchy partition. Accord status and review metadata remain separate from workflow state. The TUI surfaces structural validation warnings and errors so invalid hierarchy or unresolved required references can be corrected before work is trusted. Completion archives a task to Logs rather than creating a persistent `done` state.
...
Press `?` from any non-text view, utility panel, picker, or confirmation to open the universal keybinding reference. It groups Global, Navigation, Current view, Board actions, Validation, Logs, Rules, Decisions, Utility inbox, Dialogs and text input, and Mouse controls. Use `j`/`k`, arrows, Ctrl-U/Ctrl-D, PageUp/PageDown, Home/End, or the wheel to scroll. Use `h`/`l`, Tab/Shift-Tab, or a section click to change sections. `Esc` closes help and restores the exact underlying state. `q` quits from help.
