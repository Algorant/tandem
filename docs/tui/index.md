...
Set `badge_style` to control Board priority/status/tag chips. The default, `filled-muted`, keeps the compact badge shape with a softer fill for transparent and image-backed terminals. Other options are:

```toml
badge_style = "filled-muted" # muted filled chip, default
badge_style = "accent-rail"  # small colored rail plus label
badge_style = "text-only"    # colored label, no chip fill
badge_style = "ghost-chip"   # outlined/chip text with transparent fill
badge_style = "solid"        # legacy saturated filled block
```

The same key can also be written as `[badges] style = "ghost-chip"` in user themes, user config, or `.tandem/theme.toml`.
