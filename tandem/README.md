...
- root keys: `theme` (workspace selector), `base`, `builtin`, `extends`, `name`, `transparent_background` (optional boolean; default `false`), `badge_style` (optional; default `filled`) or `[badges] style`
...
`badge_style` controls Board priority/status/tag chips: `filled` (default softened fill), `accent` (small colored rail), `text` (colored label), `ghost` (transparent chip outline), or `solid` (legacy saturated fill).
...
- v0 commands: `--version`, `version`, `init`, `list`, `show`, `add`, `move`, `update`, `complete`, `log`, `search`, `accord`, `rules`, `decision`, `tui`.
