...
`tandem checkpoint` is the explicit native Git flush for commit/push workflows. After creating the real source commit, run it and require a clean `.tandem/` before pushing. Success returns `data.checkpoint`; a checkpoint failure exits `1` with `{"ok":false,"error":{"code":"checkpoint","message":"...","details":{"checkpoint":{...}}}}`. See `../plan/task-39-pi-handoff.md`.
