...
- Parent rows show a concise disclosure affordance plus a right-side active/logged descendant rollup. `Enter` expands/collapses task children one level at a time and opens/closes inline previews for leaf and Epic Board rows; clicking an already-selected row uses the same activation path. Space is unbound on Board, and `Tab` retains the detail pane.
...
| Surface | Keys |
| --- | --- |
| Board | `a` add, `e` edit, `b` State/Epic Board, `f` filter picker, `m` move picker, `v` Validation picker; `Enter` activates the selected row |
