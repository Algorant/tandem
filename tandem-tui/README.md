...
- `tdm tui` currently has top-level Board, Review, Logs, Rules, and Decisions tabs. `1`..`5` and mouse tab clicks switch views; Board keeps `a` quick-add and `H`/`L` moves for the selected task. Review is a read-only filtered queue with selectable rows, inspection detail, reason badges, accord/review/state/priority metadata, blockers, and CLI action hints. Logs/Rules/Decisions remain read-only placeholders/counts for now. Built-in `default-dark` theme styles apply, and `.tandem/theme.toml` can override documented color keys.
