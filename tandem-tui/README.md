...
- `tdm tui` currently starts with a Board-first shell; `a` quick-adds a task, `H`/`L` moves the selected task to the previous/next configured state, built-in `default-dark` theme styles apply, `.tandem/theme.toml` can override documented color keys, and broader MVP views/mutations remain the active focus.
