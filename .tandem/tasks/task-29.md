...
accord:
  ...
  note: "Implementation approach and observed tests accepted provisionally; one localized documentation correction before ready: docs/cli/index.md's new decision --reference sentence says repository paths belong in Markdown body, contrary to Algorant's approved relatedFiles split. State paths belong in relatedFiles metadata, not references; you may clarify that current decision CLI lacks --related-file and adding that option is out of scope. Make docs/guides/decisions.md equally direct rather than implying record ty"
