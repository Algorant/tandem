...
```text
Standalone Task global task-J
└── Subtask task-J-1                   parentRelationship: subtask
...
The first Epic-owned Task Subtask is completed and a later Subtask continues at `task-M-3`, proving per-Task suffix allocation scans logs. The standalone Task is validly reparented beneath the Decision while retaining its global ID and existing Subtask, proving canonical same-role reparenting preserves immutable IDs.
...
- The repository worker handoff spec keeps global-ID Tasks as the only delegation roots, projects their Subtasks into one worker campaign, and preserves a single Task-root settlement boundary.
...
- Reparenting a Task beneath a generic document preserves its global ID and Subtasks; reparenting that changes Task → Subtask role fails without mutation because IDs are immutable.
